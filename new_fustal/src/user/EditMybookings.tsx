...
const EditMybookings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      getSingleBookingApi(id).then((res: BookingResponse) => {
        ...
        if (Array.isArray(fromData)) {
          setSelectedTimes(
            fromData.map((time) => ({ value: time, label: time }))
          );
        } else {
  ...
  const handleTimeChange = (selectedOptions: MultiValue<TimeOption>) => {
    setSelectedTimes(selectedOptions as TimeOption[]);
  };
  ...
  const confirmChanges = () => {
    ...
    updateBookingApi(id, formData)
      .then((res: { data: { success: boolean; message: string } }) => {
        if (res.data.success) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                id="futsalName"
                readOnly
                className="mt-1 block w-full border border-gray-300 text-gray-900 rounded-lg p-2.5"
                value={futsalName}
              />
            </div>
            <div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-5 rounded-lg shadow-xl">
            ...
            <p>
              <strong>Date:</strong> {futsalDate}
            </p>
            <p>
              <strong>Time:</strong>{" "}
              {selectedTimes.map((option) => option.label).join(", ")}
            </p>
