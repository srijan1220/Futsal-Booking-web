...
//           <div className="flex-shrink-0 relative py-6 ml-auto " ref={userMenuRef}>
//             <FontAwesomeIcon
//               icon={faUser}
...
//                 <Link to={'/update'} className="block px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-600 transition duration-300" >Profile</Link>
//                 <button className="block px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-600 transition duration-300" onClick={openlogoutModal}>Logout</button>
//               </div>
...
//             <svg className="w-6 h-6 text-black hover:text-green-600"
//               fill="none"
//               strokeLinecap="round"
...
const AdminNavbar = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const userMenuRef = useRef<HTMLDivElement>(null); // Added proper type for the ref
  const [islogoutModalOpen, setlogoutIsModalOpen] = useState(false);
  const openlogoutModal = () => setlogoutIsModalOpen(true);
  const closelogoutModal = () => setlogoutIsModalOpen(false);
          ...
          <>
            ...
            <div className="flex-shrink-0 relative py-6 ml-auto" ref={userMenuRef}>
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 py-2 w-48 bg-white rounded-md shadow-xl z-50">
                  <Link to={'/update'} className="block px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-600 transition duration-300">Profile</Link>
                  <button className="block px-4 py-2 text-sm text-gray-700 hover:bg-green-50 hover:text-green-600 transition duration-300 w-full text-left" onClick={openlogoutModal}>Logout</button>
                </div>
              )}
